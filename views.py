...
def register(request):
    ...
    if newuser[0] == False:
        ...
        return redirect('/')
    if newuser[0] == True:
        messages.success(request, 'Your In, Welcome')
...
def login(request):
    ...
    if request.method == 'GET':
        ...
        if user[0] == False:
            ...
            return redirect('/')
        if user[0] == True:
            messages.add_message(request, messages.INFO,'Welcome, Logged in and all set!')
...
def quotes(request):
    ...
    print "quotes:", quotes
    favourites= Quote.objects.filter(favorite__id=request.session['id'])
    user= User.objects.get(id=request.session['id'])
    notfav= Quote.objects.exclude(favorite__id=request.session['id'])
    print "notfav:", notfav
    others= Quote.objects.all().exclude(favorite__id=request.session['id'])
...
def show(request, quote_id):
    ...
    quotes= Quote.objects.filter(favorite=creator)
    count= Quote.objects.filter(favorite=creator).count()
    print quotes
    ...
    return render(request, 'quotes/quotepost.html', context)
